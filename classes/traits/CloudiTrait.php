...
trait CloudiTrait
{
    ...
    public function getCloudiUrl($src, $id=null, $version=null)
    {
        ...
        if($formatOption) array_push($options['transformation'], $formatOption);
    ...
    public function setFormat($vers='base') {
        ...
        $array = $versions[$vers];
        if(is_numeric($width)) $array['width'] = $width;
        if(is_numeric($height)) $array['height'] = $height;
        return $array;
    ...
    public function readCloudiId($src) {
        ...
        if(!$srcPath) throw new ApplicationException('CLOUDINARY_PATH problem, refresh your page');
    ...
    {
        $srcPath = env('CLOUDINARY_PATH');
        if(!$this->getCloudiExiste($src)) return null;
        if(!$srcPath) throw new ApplicationException('CLOUDINARY_PATH problem, refresh your page');
        $modelName = snake_case((new \ReflectionClass($this))->getShortName());
        $modelSlug = $this->attributes[$this->cloudiSlug];
        $version = null;
        $hasCoudiFiles = $this->cloudis_files ? $this->cloudis_files->count() : null;
        if($hasCoudiFiles) $version = $this->cloudis_files()->where('code', $src)->get()->first()->version ?? false;
        if($version) {
    ...
    public function getCloudiExiste($src) {
        ...
        if(!$this->cloudis_files()->where('code', $src)->count()) return false;
